...
class TopupController extends BaseController
{
    ...
    public function topup()
    {
        ...
        if ($amount < 10000 || $amount > 1000000) {
            $balanceData = number_format($amount, 0, ',', '.');
            return redirect()->to('/topup')->withInput()->with('errorMessage', 'Jumlah top up harus antara 10,000 sampai 1,000,000')->with('amount', $balanceData);
        }
            ...
            switch ($statusCode) {
                case 200:
                    ...
                    if ($responseData['status'] === 0) {
                        $balanceData = number_format($amount, 0, ',', '.');
                        return redirect()->to('topup')->with('success', 'Top Up Berhasil')->with('amount', $balanceData);
                    } else {
